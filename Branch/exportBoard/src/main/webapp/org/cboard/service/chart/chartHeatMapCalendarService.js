...
cBoard.service('chartHeatMapCalendarService', function () {
    ...
    this.render = function (containerDom, option, scope, persist) {
        ...
        return new CBoardEChartRender(containerDom, option, null, scope).chart(height, persist);
    };
    ...
    this.parseOption = function (data) {
        ...
        var nowDate =echarts.format.formatTime('yyyy-MM', new Date());
        var yearMap = {};
        dates.map(function (date) {
            var time = +echarts.number.parseDate(date);
            if (time) {
                var year = echarts.format.formatTime('yyyy', time);
                nowDate = echarts.format.formatTime('yyyy-MM', time);
                yearMap[year] = 1;
            }
        });
        var yearArr = [];
...
//                max = value > max ? value : max;
        ...
        var option = {
            ...
            calendar: calendar,
            series: series,
            name: typeof(data.chartConfig.name != 'undefined')? data.chartConfig.name : ''
        };
