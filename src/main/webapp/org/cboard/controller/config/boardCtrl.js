    ...
    var loadBoardDataset = function (status) {
                ...
                callback: function (dps) {
                    ...
                    if (dps.msg == "1") {
                        ...
                        if (dataset != undefined) {
                            $scope.boardDataset.push({name: dataset.name, columns: dps.columns, datasetId: dataset.id, injectionParams: dps.injectionParams});
                        }
                        status.i--;
                    } else {
        ...
        _.each(widgetArr, function (w) {
            ...
            dataService.getColumns({
                ...
                callback: function (dps) {
                    if (dps.msg == "1") {
                        $scope.boardDataset.push({name: w.name, columns: dps.columns, widgetId: w.id, injectionParams: dps.injectionParams});
                        status.i--;
                    } else {
                        $scope.alerts = [{msg: dps.msg, type: 'danger'}];
                    }
        ...
        $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                $scope.param_types = [
                    ...
                    {name: translate('CONFIG.DASHBOARD.PARAM_TYPE_TEXTOR'), value: 'textor'},
                    {name: translate('CONFIG.DASHBOARD.PARAM_TYPE_DATEPICKER'), value: 'datepicker'}
                ];
                $scope.status = status;
                $scope.param = param;
                $scope.type = ['=', '>', '<', '≥', '≤'];
                ...
                $scope.foldCube = function (cube, e) {
                    ...
                    if (e.target.className == "cubeName ng-binding" || e.target.localName == 'img') {
                        if (node.style.height == "25px" || node.style.height == "") {
                            node.style.height = 25 * (cube.columns.length + cube.injectionParams.length + 1) + "px";
